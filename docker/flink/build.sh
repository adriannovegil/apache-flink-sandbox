...
if [[ $(uname -i) == 'aarch64' ]]; then
  ...
  cp docker/flink/Dockerfile Dockerfile-ARM
...
docker build --build-arg flink_dist="${FLINK_DIST}" --build-arg job_artifacts="${JOB_ARTIFACTS_TARGET}" --build-arg hadoop_jar="${SHADED_HADOOP}" --build-arg python_version="${PYTHON_VERSION}" -t "${IMAGE_NAME}" -f ./docker/flink/$DOCKERFILE .
